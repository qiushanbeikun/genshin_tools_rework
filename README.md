...
- `node-sass` has been replaced by `sass` to solve incompatible issues.
...
- Inside the `backend` folder, do the following:
  - Create a copy of `genshin_tools_rework/settings/local.py.example`:  
  `cp genshin_tools_rework/settings/local.py.example genshin_tools_rework/settings/local.py`
  - Create a copy of `.env.example`:
  `cp .env.example .env`

### If you are using Docker:
- Open the `/backend/.env` file on a text editor and uncomment the line `DATABASE_URL=postgres://genshin_tools_rework:password@db:5432/genshin_tools_rework` to connect to a local Postgres DB.
...
- Open the `/backend/.env` file on a text editor and do one of the following:
  - This project is using PostgreSQL, if you want run on a local DB, uncomment and edit the line `DATABASE_URL=postgres://genshin_tools_rework:password@db:5432/genshin_tools_rework` in order to make it correctly point to your database URL
...
- Open a new command line window and go to the project's directory
- Create a new virtualenv with either [virtualenvwrapper](https://virtualenvwrapper.readthedocs.io/en/latest/) or only virtualenv: `mkvirtualenv genshin_tools_rework` or `python -m venv genshin_tools_rework-venv`
...
- Make sure the virtualenv is activated `workon genshin_tools_rework` or `source genshin_tools_rework-venv/bin/activate`
- Run `.\Genshin_tools_rework_venv\Scripts\activate` to enable virtual environment
...
- `workon genshin_tools_rework` or `source genshin_tools_rework-venv/bin/activate` depending on if you are using virtualenvwrapper or just virtualenv.
